...
```
    ...
    <ipython-input-3-7675ea8345e2> in <module>
          1 # 원본 자체를 지우고자 한다면...
          2 # 방법 2 : inplace 파라미터를 할용 True인 경우 원본데이터에 수행
    ----> 3 cust.drop('r3m_avg_bill_amt10', axis=1, inplace=True)
...
```
    ...
    <pandas.core.groupby.generic.DataFrameGroupBy object at 0x7eff0b490cc0>
...
```
    ...
    C:\Users\10094526\AppData\Local\Temp\ipykernel_61944\2424793971.py:2: FutureWarning: ['age', 'dt_stop_yn', 'npay_yn', 'termination_yn'] did not aggregate successfully. If any error is raised this will raise in a future version of pandas. Drop these columns/ops to avoid this warning.
      cust.set_index(['cust_class','sex_type']).groupby(level=[0,1]).aggregate([np.mean, np.max])
...
```
    ...
    ~\AppData\Local\Temp\ipykernel_61944\368441849.py in <module>
    ...
    C:\ProgramData\Anaconda3\lib\site-packages\pandas\core\reshape\reshape.py in __init__(self, index, level, constructor)
        138             )
        139 
...
```
    ...
    ~\AppData\Local\Temp\ipykernel_61944\528317783.py in <module>
          1 # index중복이있는 경우 error가 남
    ...
    C:\ProgramData\Anaconda3\lib\site-packages\pandas\core\reshape\concat.py in <listcomp>(.0)
        611         ndim = self._get_result_dim()
        612         return [
    --> 613             self._get_concat_axis if i == self.bm_axis else self._get_comb_axis(i)
        614             for i in range(ndim)
    ...
    C:\ProgramData\Anaconda3\lib\site-packages\pandas\_libs\properties.pyx in pandas._libs.properties.CachedProperty.__get__()
    ...
    C:\ProgramData\Anaconda3\lib\site-packages\pandas\core\reshape\concat.py in _maybe_check_integrity(self, concat_index)
...
```python
pd.merge(customer, orders, on='cust_id', how='inner').groupby(['name', 'item']).sum().loc['영희'].sort_values(by='quantity', ascending=True)
```
